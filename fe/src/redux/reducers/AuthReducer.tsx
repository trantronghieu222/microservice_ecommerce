...
export const loginActionApi = (login: LoginModelType) => {
    return async (dispatch: DispatchType) => {
        try {
            const res = await axios.post("http://localhost:8080/auth-service/auth/login", login);
...
export const registerActionApi = (register: RegisterModelType) => {
    return async (dispatch: DispatchType) => {
        try {
            const res = await axios.post("http://localhost:8080/auth-service/auth/register", register);
